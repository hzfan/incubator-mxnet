...
cdef NewArray(NDArrayHandle handle, int stype=-1, int is_np_op):
...
cdef class CachedOp:
    ...
    def __call__(self, *args, out=None):
        ...
        if num_output == 1:
            return NewArray(p_output_vars[0], p_output_stypes[0], is_np_sym)
        else:
            return [NewArray(p_output_vars[i], p_output_stypes[i], is_np_sym) for i in range(num_output)]
